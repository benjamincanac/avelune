...
3. **One arena, bundled.** The playable footprint (sand disc + solid tile ring) is `generateHub()` in `shared/utils/maze.ts`; the stadium bowl around it is procedural three.js in `app/utils/stadium.ts`, built from the same constants by every client; any hand-placed kit piece rides `shared/data/hub-structure.json` / `hub-props.json`. Nothing about the arena travels over the socket, only players. It is a constant: no seeds, no per-floor generation, no reset.
