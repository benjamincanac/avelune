...
- [x] **Character creator** (`CharacterGate`): gender × outfit (Peasant/Ranger) × hairstyle × beard × outfit colorway, name, Randomise, live draggable 3D turntable on a fixed camera, so a taller character reads taller. Runtime cloth-only recolor (`app/utils/appearance.ts`)
...
- [x] Knight, Noble and Wizard outfits from the purchased CC0 source pack (`modular-character-outfits-source`), three colourways each, 18 roster GLBs in total. The Knight's closed helm is built without hair as one GLB per gender (`hairless` in `characters.ts`, the creator hides the hair picker), the Noble keeps a crown over the hair, the Wizard is bareheaded because the pack has no hat
...
- [ ] **Verify by eye**: the dragon on its orbit, ghosts at night, and which way each critter species faces. Headed passes confirmed critters spawn, walk, idle and stay off roofs, and the heading maths matches the Oracle's `MushroomKing` from the same pack, but no close-up was captured
