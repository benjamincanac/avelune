...
- Quaternius packs download from Google Drive folders linked on quaternius.com pack pages (`gdown --folder`); the Universal characters are itch.io-only behind Cloudflare (manual download), and the outfits come from the purchased source pack `modular-character-outfits-source`; then run `convert_universal_characters.py` with `ONLY=` for a subset
