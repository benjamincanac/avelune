...
- `app/utils/{textures,stadium,composeColosseum,appearance,characterModels}.ts` — three.js helpers; they touch `document` and are only imported from the client components above.
...
- Cross-layer state (chat bubbles, Oracle speech, editor state) flows through Nuxt `useState` composables (`useGame`, `useOracle`, `useEditor`), not props into the canvas.
