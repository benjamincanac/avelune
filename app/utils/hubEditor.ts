...
interface EditorState {
  ...
  dirty: Ref<boolean>
  /** Snapshot the current placements as an undo step (also marks dirty). */
  commit: () => void
  undo: () => void
  redo: () => void
}
...
export function createHubEditor(opts: HubEditorOptions): HubEditor {
  ...
  let looking = false
  // Drag is armed on mousedown-over-a-piece but only *activates* once the pointer
  // moves past a threshold, so a plain click selects without moving anything. On
  // activation we record a grab offset (piece pos − ground hit) so the piece
  // tracks the cursor by delta instead of teleporting its origin onto the ray.
  ...
  function onMouseDown(e: MouseEvent) {
    ...
    if (kind) {
      // Stamp a new prop where the ground was clicked; keep the kind armed.
      const g = groundHit()
      if (!g) return
    ...
    const idx = pickIndex()
    editor.selected.value = idx
    // Arm (don't start) a drag; a click that never moves just selects.
    dragCandidate = idx
    dragging = false
  ...
  function onMouseMove(e: MouseEvent) {
    ...
    if (dragCandidate == null || !setRay(e)) return
    const p = editor.placements.value[dragCandidate]
    const obj = clones[dragCandidate]
    if (!p || !obj) return
    // Drag in a horizontal plane at the piece's current height, so roofs/upper
    // walls stay at their elevation.
    const elev = p.z ?? 0
    dragPlane.constant = -elev
    if (!raycaster.ray.intersectPlane(dragPlane, hit)) return
    if (!dragging) {
      // Only begin dragging past the threshold; then grab from the current point
      // so the piece doesn't jump onto the cursor.
  ...
  function onMouseUp(e: MouseEvent) {
    ...
    const idx = dragCandidate
    const wasDragging = dragging
    dragCandidate = null
    dragging = false
    if (!wasDragging || idx == null) return
    const p = editor.placements.value[idx]
    const obj = clones[idx]
    if (p && obj) {
      p.x = Math.round(obj.position.x * 1000) / 1000
      p.y = Math.round(obj.position.z * 1000) / 1000
      editor.commit()
    }
  }
  ...
  function onKeyDown(e: KeyboardEvent) {
    ...
    if (e.code === 'Escape') {
      if (editor.selected.value != null) editor.selected.value = null
      else editor.paletteKind.value = null
    }
    else if ((e.code === 'Delete' || e.code === 'Backspace') && i != null) {
